                ...
                <div class="col-lg-6 col-7">
                    <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                        <ol class="breadcrumb breadcrumb-links">
                            ...
                            <li class="breadcrumb-item"><a href="/account/dashboard">Dashboard / Brands</a></li>
                ...
                <div class="card-header border-0">
                    <h2 class="mb-0"> <i class="fas fa-calendar"></i> My {{ $titles }}
                        <span class="float-right">
                            <a href="{{ route('account.brand.create') }}" class="btn btn-primary btn-sm">
                    ...
                    <table class="table align-items-center table-flush">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$brand->name}}</td>
                                <td>{{$brand->desc}}</td>
                                <td>{{$brand->owner->username}}</td>
                                <td>
                                    @php

                                     $route_param = array();
                                     $route_param[$index] = $brand->id;
                                    @endphp
                                    <span class="badge badge-dot mr-4">
                                        ...
                                        <a href="" onclick="event.preventDefault(); document.getElementById('delete-brand-{{ $brand->id }}').submit();"><span class="badge badge-danger" >Delete</span>
                                        </a>
                                        <form action="{{ route('account.brand.destroy',$brand->id) }}" method="POST" id="delete-brand-{{ $brand->id }}" style="display: none;">
                                            @csrf
                                            @method('DELETE')
